...
class TrustTunnelDeepLinkService {
  ...
  ServerConfig parse(String value, {ServerConfig? baseConfig}) {
    final normalized = _normalizeInput(value);
    if (!normalized.toLowerCase().startsWith(_schemePrefix)) {
      throw const FormatException(
        'Expected TrustTunnel deep link starting with tt://?',
      );
    }
    ...
    if (addresses.isEmpty) {
      throw const FormatException(
        'Deep link does not contain endpoint addresses',
      );
    }
    ...
    return base.copyWith(
      ...
      username: username,
      password: password,
      skipVerification: _readBool(
        fields,
        _tagSkipVerification,
        defaultValue: false,
      ),
  ...
  Map<int, List<Uint8List>> _parseFields(Uint8List bytes) {
    ...
    while (offset < bytes.length) {
      ...
      if (offset + lengthValue.value > bytes.length) {
        throw const FormatException(
          'Deep link field length exceeds payload size',
        );
      }
  ...
  String _readUtf8(Map<int, List<Uint8List>> fields, int tag) {
    ...
    if (value == null || value.isEmpty) {
      throw FormatException(
        'Deep link is missing required field 0x${tag.toRadixString(16)}',
      );
    }
    return value;
  }
  ...
  String _readUpstreamProtocol(Map<int, List<Uint8List>> fields) {
    ...
    switch (entries.last.first) {
      ...
      default:
        throw const FormatException(
          'Unsupported upstream protocol in deep link',
        );
    }
  }
  ...
  List<Uint8List> _splitDerCertificates(Uint8List chain) {
    ...
    while (offset < chain.length) {
      if (chain[offset] != 0x30) {
        throw const FormatException(
          'Invalid DER certificate chain in deep link',
        );
      }
      ...
      if (offset + objectLength > chain.length) {
        throw const FormatException(
          'DER certificate extends beyond payload size',
        );
      }

      certificates.add(
        Uint8List.fromList(chain.sublist(offset, offset + objectLength)),
      );
      offset += objectLength;
    }
  ...
  _AddressEntry _parseAddressEntry(String value) {
    ...
    if (separatorIndex <= 0 || separatorIndex == trimmed.length - 1) {
      throw FormatException(
        'Endpoint address must be in host:port format: $trimmed',
      );
    }
...
class _VarintValue {
  ...
  const _VarintValue({required this.value, required this.byteLength});
}
...
class _AddressEntry {
  ...
  const _AddressEntry({required this.host, required this.port});
}
