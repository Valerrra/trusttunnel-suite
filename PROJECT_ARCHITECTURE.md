...
- хороший референс по информационной архитектуре панели;
- стек: Go, Gin, embedded templates/assets, SQLite, i18n, scheduled jobs;
- подходит как ориентир для TrustTunnel WebUI по разделам:
  - overview;
  - users/clients;
...
Вывод: для TrustTunnel WebUI стоит брать идеи и паттерны, но учитывать GPLv3.

## 3. Лицензионная стратегия

### Безопасный вариант

- `apps/webui` пишется отдельно, по мотивам UX и структуры `3x-ui`, без копирования исходников;
...
- прямое заимствование кода из `3x-ui` тянет GPLv3-обязательства на WebUI и, возможно, на производные части.

На старте выбираем безопасный вариант.
...
Для первой версии не надо пытаться повторить всю сложность `3x-ui`. Нужен узкий TT-ориентированный MVP.
...
- FI-сервер используется как dev/staging;
- доступы берутся из `/mnt/d/VPN/access_list.MD`;
...
4. Прямое копирование `3x-ui` может создать лицензионную проблему.
5. Сертификаты и домен критичны для гладкого UX.
